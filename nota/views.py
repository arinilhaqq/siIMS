...
from nota.forms import NotaBarangForms, NotaGabunganForms
...
def nota_list(request):
    if is_authenticated(request):
        if request.session['jabatan'] == 'Akuntan' or request.session['jabatan'] == 'Admin' or request.session['jabatan'] == 'Service Advisor' or request.session['jabatan'] == 'Inventori' or request.session['jabatan'] == 'Teknisi':
            cursor = connection.cursor()
            cursor.execute("SET search_path TO public")  
            id_nota = 0
            id_appointment_nota= {}
...
def barang_view(request, id):
    if is_authenticated(request):
        if request.session['jabatan'] == 'Akuntan' or request.session['jabatan'] == 'Admin' or request.session['jabatan'] == 'Inventori' or request.session['jabatan'] == 'Teknisi' or request.session['jabatan'] == 'Service Advisor':
            nota_gabungan = NotaGabungan.objects.get(id=id)
...
def ceklis_notabarang(request, id):
    if is_authenticated(request):
        if request.session['jabatan'] =='Akuntan' or request.session['jabatan'] == 'Admin' or request.session['jabatan'] == 'Teknisi' or request.session['jabatan'] == 'Inventori' or request.session['jabatan'] == 'Service Advisor':
            nota_gabungan = NotaGabungan.objects.get(id=id)
            nota_barang = nota_gabungan.nota_barang
